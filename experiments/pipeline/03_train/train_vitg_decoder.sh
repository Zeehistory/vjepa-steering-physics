...
#SBATCH --output=logs/vitg_dec_%j.out
#SBATCH --error=logs/vitg_dec_%j.err
...
requeue() { echo "[vitg_dec] USR1/timeout -> requeue $SLURM_JOB_ID"; scontrol requeue $SLURM_JOB_ID; exit 0; }
trap requeue USR1
BASE=.
LATENT_DIR=$BASE/outputs/latents/moving_ball_scene_angvel2d_vitg_train/test/vjepa2_giant
OUTPUT_DIR=$BASE/outputs/runs/moving_ball_scene_angaccel2d_vitg_decoder
...
if [ -d "$CKDIR" ]; then
    ...
    [ -n "$LATEST" ] && RESUME="train.resume=$LATEST" && echo "[vitg_dec] resuming from $LATEST"
fi
echo "[vitg_dec] CONFIG=$CONFIG OUT=$OUTPUT_DIR MAX_STEPS=$MAX_STEPS LAT=$LATENT_DIR"
df -h . | tail -1
...
wait $CHILD
RC=$?
echo "[vitg_dec] training process exited rc=$RC"
df -h . | tail -1
exit $RC
