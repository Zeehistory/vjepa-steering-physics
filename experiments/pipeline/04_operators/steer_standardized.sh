#!/bin/bash
#SBATCH --job-name=steer_std
#SBATCH --requeue
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=180G
#SBATCH --time=08:00:00
#SBATCH --output=logs/steer_std_%j.out
#SBATCH --error=logs/steer_std_%j.err

# STEERING ONLY. Scores the already-fitted baseline vs standardized command operators
# (written by refit_standardized.sh) head-to-head through the decoder on the 100-scene test split.
#
