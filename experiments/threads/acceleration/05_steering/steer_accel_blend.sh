#!/bin/bash
#SBATCH --job-name=ablend_ap
#SBATCH --gres=gpu:1
#SBATCH --requeue
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --time=04:00:00
#SBATCH --output=logs/ablend_ap_%j.out
#SBATCH --error=logs/ablend_ap_%j.err
#
...
python -u experiments/threads/acceleration/05_steering/steer_accel_spline.py \
    ...
    --manifold_dir "$ANA/manifold" --manifold_variants man \
    --manifold_knots "" --manifold_blend "" \
    --hybrid_blend_srcs man,spline8 \
    --hybrid_blend_lams 0,0.15,0.3,0.45,0.6,0.75,0.9,1.0 \
    --checkpoint "$BASE/outputs/runs/moving_ball_scene_accel2d_mixed_decoder_fp/checkpoints/last.pt" \
    --output_dir "$ANA/steer_blend_ap$SHARD" \
...
echo "[ablend_ap] shard $SHARD done (scenes $START..$((START+PER-1)))"
