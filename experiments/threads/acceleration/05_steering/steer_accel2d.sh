#!/bin/bash
#SBATCH --job-name=accel_steer
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=60G
#SBATCH --time=02:00:00
#SBATCH --output=logs/accel_steer_%j.out
#SBATCH --error=logs/accel_steer_%j.err
...
CK="${1:-last}"
OUTTAG="${2:-$CK}"
CKPT=$BASE/outputs/runs/moving_ball_scene_accel2d_decoder_fp/checkpoints/${CK}.pt
python -u experiments/threads/acceleration/05_steering/steer_accel2d.py \
    ...
    --checkpoint "$CKPT" \
    --output_dir $BASE/outputs/analysis/moving_ball_accel2d/steer_${OUTTAG} \
    --ks 2,4,8,16 --num_scenes ${NUM_SCENES:-100} --cmd_scales "${CMD_SCALES:-1.0,1.5,2.0,2.5,3.0}" \
    --cmd_ku ${CMD_KU:-8} \
    --device cuda
RC=$?
echo "[accel_steer] steer exit=$RC ckpt=$CKPT out=steer_${OUTTAG}"

SUM=$BASE/outputs/analysis/moving_ball_accel2d/steer_${OUTTAG}/steer2d_summary.json
if [ -f "$SUM" ]; then
    python -u experiments/pipeline/04_operators/calibrate_cmd_gain.py --summary "$SUM" --val_frac 0.5 \
        --out $BASE/outputs/analysis/moving_ball_accel2d/steer_${OUTTAG}/calib_cmd_gain.json
fi
echo "[accel_steer] done (exit $RC)"
