#!/bin/bash
#SBATCH --job-name=aman_ap
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --time=10:00:00
#SBATCH --output=logs/aman_ap_%j.out
#SBATCH --error=logs/aman_ap_%j.err
#
...
python -u experiments/threads/acceleration/05_steering/steer_accel_spline.py \
    ...
    --manifold_dir "$MANDIR" --manifold_variants "$VARIANTS" \
    --manifold_knots 3 --manifold_blend 8 \
    --bigU_dir "$ANA/subspace_bigU" --bigU_ks 8,32,128 \
    --checkpoint "$CKPT" \
    --output_dir "$ANA/steer_man_ap$SHARD" \
    --pairs all --scene_start "$START" --num_scenes "$PER" \
    --knots 3,8 \
    --gains 1.0,1.5,2.0,2.5,3.0,3.5,4.0,5.0,6.0 \
    --oracle_gains 1.0,1.5,2.0,2.5,3.0,4.0 \
    --oracle_arms full_delta,prof_full \
    --family_scenes 0 --traj_scenes 2 --device cuda
echo "[aman_ap] shard $SHARD done (scenes $START..$((START+PER-1)))"
