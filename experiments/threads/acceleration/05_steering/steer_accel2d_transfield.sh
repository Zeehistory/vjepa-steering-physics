#!/bin/bash
#SBATCH --job-name=amix_tfst
#SBATCH --gres=gpu:b200:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=02:00:00
#SBATCH --output=logs/accelmix_tfsteer_%j.out
#SBATCH --error=logs/accelmix_tfsteer_%j.err
# DECODE the 2nd-order translation-field accel operator + leakage-free gain calibration.
# Read steer_transfield/calib_cmd_gain.json HELDOUT vs canon 14.46 + full_delta 10.07 ceiling.
...
python -u experiments/threads/acceleration/05_steering/steer_accel2d.py \
    ...
    --checkpoint "$CKPT" \
    --output_dir $BASE/outputs/analysis/moving_ball_accel2d_mixed/steer_transfield \
    --ks 8 --num_scenes ${NUM_SCENES:-100} \
    --cmd_scales "${CMD_SCALES:-1.0,1.5,2.0,2.5,3.0,4.0}" \
    --features transfield --viz_scenes 4 --viz_gain 2.0 --device cuda
SUM=$BASE/outputs/analysis/moving_ball_accel2d_mixed/steer_transfield/steer2d_summary.json
[ -f "$SUM" ] && python -u experiments/pipeline/04_operators/calibrate_cmd_gain.py --summary "$SUM" --val_frac 0.5 \
    --out $BASE/outputs/analysis/moving_ball_accel2d_mixed/steer_transfield/calib_cmd_gain.json
echo "[amix_tfsteer] DONE"
