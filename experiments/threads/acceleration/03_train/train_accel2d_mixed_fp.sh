...
#SBATCH --output=logs/accelmix_fp_train_%j.out
#SBATCH --error=logs/accelmix_fp_train_%j.err
...
requeue() { echo "[amix_fp] USR1/timeout -> requeue $SLURM_JOB_ID"; scontrol requeue $SLURM_JOB_ID; exit 0; }
trap requeue USR1

BASE=.
LATENT_DIR=$BASE/outputs/latents/moving_ball_scene_accel2d_mixed/train/vjepa2_large
OUTPUT_DIR=$BASE/outputs/runs/moving_ball_scene_accel2d_mixed_decoder_fp
...
if [ -d "$CKDIR" ]; then
    ...
    [ -n "$LATEST" ] && RESUME="train.resume=$LATEST" && echo "[amix_fp] resuming from $LATEST"
fi

echo "[amix_fp] CONFIG=$CONFIG OUT=$OUTPUT_DIR MAX_STEPS=$MAX_STEPS LAT=$LATENT_DIR"
...
wait $CHILD
RC=$?
echo "[amix_fp] training process exited rc=$RC"
exit $RC
