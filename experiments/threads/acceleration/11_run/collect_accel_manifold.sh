#!/bin/bash
#SBATCH --job-name=aman_collect
#SBATCH --cpus-per-task=2
#SBATCH --mem=24G
#SBATCH --time=00:40:00
#SBATCH --output=logs/aman_collect_%j.out
#SBATCH --error=logs/aman_collect_%j.err
#
...
SUMS=$(ls "$ANA"/steer_man_ap[0-9]/steer2d_summary.json \
...
for BASE_ARM in spline_K8_s2.5 full_delta_s1; do
    ...
    python -u experiments/threads/acceleration/04_operators/calibrate_spline_gain.py --summaries $SUMS --val_frac 0.5 \
        --baseline "$BASE_ARM" --out "$ANA/calib_manifold_vs$TAG.json" || true
done
echo "[collect] done -> $ANA/calib_manifold_vs*.json"
