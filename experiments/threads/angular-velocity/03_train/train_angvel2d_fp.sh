#!/bin/bash
#SBATCH --job-name=angvel_fp_train
#SBATCH --requeue
#SBATCH --gres=gpu:1
...
#SBATCH --output=logs/angvel_fp_train_%j.out
#SBATCH --error=logs/angvel_fp_train_%j.err
...
requeue() { echo "[angvel_fp] USR1/timeout -> requeue $SLURM_JOB_ID"; scontrol requeue $SLURM_JOB_ID; exit 0; }
trap requeue USR1
BASE=.
LATENT_DIR=$BASE/outputs/latents/moving_ball_scene_angvel2d/train/vjepa2_large
OUTPUT_DIR=$BASE/outputs/runs/moving_ball_scene_angvel2d_decoder_fp
...
if [ -d "$CKDIR" ]; then
    ...
    [ -n "$LATEST" ] && RESUME="train.resume=$LATEST" && echo "[angvel_fp] resuming from $LATEST"
fi
echo "[angvel_fp] CONFIG=$CONFIG OUT=$OUTPUT_DIR MAX_STEPS=$MAX_STEPS LAT=$LATENT_DIR"
...
wait $CHILD
RC=$?
echo "[angvel_fp] training process exited rc=$RC"
exit $RC
