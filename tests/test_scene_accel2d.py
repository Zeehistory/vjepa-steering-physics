"""Regression tests for the 2D-acceleration scene variant (scene_accel2d, Step 2 "Bravo").

Within a scene every clip shares ONE initial position AND ONE initial velocity v0; each rank has a
distinct constant 2D ACCELERATION vector a. Because frame 0 and the initial velocity are identical across
the scene, H_b - H_a isolates Delta a (the trajectory curvature) with start, initial velocity, and
appearance all cancelled. We assert that contract, that the acceleration is genuinely constant and
recoverable by a parabola fit, and that every quadratic path stays in frame.
"""
...
def _gen(seed: int = 0):
    return MovingBall(image_size=128, num_frames=16, fps=4, scenario="scene_accel2d",
...
def test_shared_initial_velocity():
    """Frame-0 velocity v0 is identical across all ranks (only the acceleration varies)."""
    clips, _ = _scene()
    v0 = {(round(float(_col(c, "obj0_vel_x")[0]), 9), round(float(_col(c, "obj0_vel_y")[0]), 9))
          for c in clips}
    assert len(v0) == 1, f"initial velocity must be shared across ranks: {v0}"


def test_eight_distinct_acceleration_vectors():
    """Each rank has a distinct constant 2D acceleration; both direction and magnitude vary."""
    clips, _ = _scene()
    accs = {(round(c.meta["acc_x"], 7), round(c.meta["acc_y"], 7)) for c in clips}
    assert len(accs) == K, f"expected {K} distinct acceleration vectors, got {len(accs)}"
    dirs = {round(float(np.arctan2(c.meta["acc_y"], c.meta["acc_x"])), 4) for c in clips}
    mags = {round(float(np.hypot(c.meta["acc_x"], c.meta["acc_y"])), 7) for c in clips}
    assert len(dirs) == K, "acceleration directions must vary across ranks"
    assert len(mags) == K, "acceleration magnitudes must vary across ranks"
...
def test_velocity_evolves_linearly_with_acceleration():
    """Per-frame velocity follows v_t = v0 + a*t (the discrete constant-acceleration roll-out)."""
    clips, _ = _scene()
    for c in clips:
...
def test_position_is_quadratic_and_parabola_recovers_accel():
    """Fitting a parabola to the GT centroid recovers a = 2*c2 (the metric the tracker uses)."""
    clips, _ = _scene()
    for c in clips:
...
def test_scene_accels_metadata_consistent():
    """Every rank carries the same scene-level acceleration set, and its own a matches its rank entry."""
    clips, _ = _scene()
    sa = [c.meta["scene_accels"] for c in clips]
    assert all(np.allclose(s, sa[0]) for s in sa), "all ranks must share the scene acceleration set"
...
def test_acceleration_varies_across_scenes():
    """Across scenes the acceleration set actually changes (not a fixed pattern)."""
    first = {tuple(np.round(np.ravel(_scene(scene_idx=s)[0][0].meta["scene_accels"]), 6))
             for s in range(10)}
    assert len(first) > 1, "acceleration sets should vary across scenes"
